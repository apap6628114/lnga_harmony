...
（13.9 KB / 19.0 KB）。**普通视频附件（`.mp4`）本轮也已实测**（2026-09，4 个真实附件
4/4 全部 200）：

| 视频 | 尺寸段 | `.thumb.jpg` | `.thumb_s.jpg` | `.medium.jpg` |
| --- | --- | --- | --- | --- |
| `…T6wSk0-zk.mp4` | 720×1280 | 320×568（25.9 KB） | 130×231 | 640×1137 |
| `…T6wS1hc-140.mp4` | 1920×1440 | 320×240（9.4 KB） | 130×97 | 640×480 |
| `…T6wS140-1hc.mp4` | 1440×1920 | 320×426（28.4 KB） | 130×173 | 640×853 |
| `…T6wSqu-1ni.mp4` | 966×2142 | 320×709（36.0 KB） | 130×288 | 640×1419 |

封面是真实 JPEG 静帧（`image/jpeg`），**宽高比与视频完全一致**
（320×568 vs 720×1280 → 0.5634 vs 0.5625），所以"视频帖也有预览图"在技术路径上完全可行。
若产品决定要做，三件事缺一不可：

1. 扩展名白名单放宽到含 `mp4`（仍排除 zip/rar 等）；
2. 加一个 **mp4 → 封面 URL** 的派生（整串追加 `.thumb.jpg`，规则见 §5.1）；
3. 尺寸从**裸名原图 URL** 解，而不是从封面 URL 解 —— 档位 URL 尾部是 `.thumb.jpg`，
   `parsePreviewSize` 对它刻意失配（见 §6.2.3）。

**当前未实现**：这是产品决策（列表里显示静帧是否可接受），不是能力缺口。
...
`ImageSizeUtil` 的进程级尺寸 LRU 在此仅作**兜底**：尺寸段解析失败时可用，
但实测覆盖率 100%，生产路径几乎不会走到。

#### 6.2.3 尺寸段是**全工程**的出口：视频附件与正文图片同样适用

图片的尺寸段规律对**视频附件**同样成立，且共用同一个函数 `parsePreviewSize()`
（正则、边界守卫、`null` 语义全部一致，不存在第二套解析）。

实测方法（2026-09）：从真实帖子取 6 个 mp4 附件（一个动态照片帖 + 一个视频帖），整文件下载后
解析 `moov/trak/tkhd` 的宽高与 3×3 旋转矩阵，还原出**显示尺寸**，再与文件名尺寸段比对：

| 附件 | 尺寸段 | 解码值 | tkhd 编码尺寸 | 旋转 | 显示尺寸 | 一致 |
| --- | --- | --- | --- | --- | --- | --- |
| `…T6wSk0-zk.mp4` | `Sk0-zk` | 720×1280 | 1280×720 | **90°** | 720×1280 | ✅ |
| `…T6wS1hc-140.mp4` | `S1hc-140` | 1920×1440 | 1920×1440 | 0° | 1920×1440 | ✅ |
| `…T6wS1hc-140.mp4`（同帖另一条） | `S1hc-140` | 1920×1440 | 1920×1440 | 0° | 1920×1440 | ✅ |
| `…T6wS140-1hc.mp4` | `S140-1hc` | 1440×1920 | 1920×1440 | **270°** | 1440×1920 | ✅ |
| `…T6wS1hc-140.mp4` | `S1hc-140` | 1920×1440 | 1920×1440 | 0° | 1920×1440 | ✅ |
| `…T6wSqu-1ni.mp4` | `Squ-1ni` | 966×2142 | 966×2142 | 0° | 966×2142 | ✅ |

**关键结论：尺寸段编码的是「旋转后的显示尺寸」，不是编码尺寸**——两个带旋转元数据的样本都
证明了这一点（编码 1280×720 + `rotate 90°` 的文件，尺寸段是 720×1280）。这正好等价于
`VideoSizeUtil.applyOrientation()` 对 `AVMetadataExtractor` 结果的修正语义，因此尺寸段可以
**直接替代**元数据探测，不会出现横竖颠倒。
...
| 位置 | 改动前 | 改动后 |
| --- | --- | --- |
| 正文视频（`BBCodeContentView`） | `aboutToAppear` 对**所有**视频发起 `AVMetadataExtractor` 元数据请求（并发上限 4），首帧 16:9 兜底、回来后才改 `aspectRatio` → 高度跳变 | 尺寸段同步解出（零请求），首帧即正确比例；只有解不出时才走元数据兜底 |
| 视频附件（`PostAttachments`） | 同上，展开附件时才异步取尺寸 | 同一次点击回调内同步填好，展开首帧即正确比例 |
| 正文图片（`BBCodeContentView`） | 图片容器 `.aspectRatio()` 以 `onComplete` 回填为**唯一**来源 → 加载完成时高度跳变 | `aboutToAppear` 按「进程缓存 → 尺寸段」预填，`onComplete` 只在两者都落空时生效 |

**「不回落到预热请求」的原则不变**：图片侧的 Range 预取会让请求数翻倍、易触发服务端节流，
尺寸段已能覆盖绝大多数 NGA 附件；预热只应作为最后手段，不得因为"想更快拿到尺寸"而引入。

**档位 URL 会失配，这是刻意的**：`xxx.mp4.thumb.jpg` 的尺寸段后面还跟着 `.thumb`，不满足
"紧贴扩展名"，`parsePreviewSize` 返回 `null`。档位图的真实尺寸与源图不同，要拿源图尺寸
必须传**裸名原图 URL**（列表侧即 `attachs` 里的原始 `attachurl`）。
...
> **角标配色配方已无引用点**：预览区角标全部移除后，`AppColors.movingPhotoBadgeBg`（`#80000000`，
> 50% 黑）+ `movingPhotoBadgeBorder` 在预览图链路不再被引用；这对常量本身**不是死资源**——
> 正文动态图角标仍在使用（`BBCodeContentView.ets:882`、`MovingPhotoPlayer.ets:563`），故保留。
> 原「不要复用 `imageViewerButtonBg`」的结论（`#33FFFFFF` 20% 白在全屏深色查看器里合适，
> 叠在浅色缩略图上白字几乎不可读）仍然成立：将来若再加角标请沿用 `movingPhotoBadge*`。
...
- **撤销的资源**：`AppColors.cardStroke` 与 `app.color.card_stroke`（base / dark）**已删除**——
  该色值只为卡片描边存在，留着就是死资源。
- **保留的部分**：预览图链路（比例驱动高度 / 多图横排）与区块间距统一
