...
**`materialColor` 必须与页面底色拉开明度差**——这是"亮色模式缺少玻璃感"的根本原因：
亮色主题原配方用的是暖白 `#73FEFAF6`，与页面底色 `#FEFAF6` 几乎同色，玻璃"溶解"在背景里、
只剩边缘一点折射，看着就是一块平白板；改成偏暗的 `#4D5A4632` 后，玻璃成为比底色更深的一块
面板，层次才立得住。暗色主题的 `#73121214` 本来就比深色底色更暗，所以**同一套参数在暗色下
天生观感更好**：玻璃与背景有明确明度差，且材质的边缘高光 / 折射都是亮部细节，在暗背景上
对比度最高。这条差异是材质本身的物理特性，不是参数没调对。
