...
23. `[B12]` 传一张图 → 把 `[img]` 从正文删掉 → 清空正文 → 关闭 → **不弹确认框**、不留空草稿；
    再次打开编辑器时是干净的
24. `[B13]`（**§9.13 已反转本条的判据，按新语义验**）**编辑**一个已有楼层：面板正文必须等于
    **服务端原文**（即使本地还留着上次编辑写下的草稿）；插一张图 → 关闭 → 确认框文案必须是
    「放弃修改？本次修改尚未保存，关闭后将丢失」——**不得**出现"已保存为草稿，下次编辑可恢复"
    → 再进入同一楼层的编辑面板 → 正文回到服务端原文（新插的图不在，属预期）
25. `[§9.13]` 编辑带图正流程：编辑里插一张图 → **不关闭**直接「保存」→ 帖子里的新图正常显示
    （凭证在本次会话内仍有效）；且 `filesDir/draft_media/` 下**不得**多出该编辑键的目录
26. `[§9.13]` 不做任何修改直接关闭编辑面板 → **不弹确认框**；只切换匿名开关后关闭 → 弹框
...
| B13 | EDIT 模式不接回草稿正文（新插图重开后从正文消失） | ✅ 已修（§9.12）→ **判据已按 §9.13 反转**：编辑模式以服务端原文为准，不读也不写草稿 |
...
`savedDraftDialogController`（新建模式；当时叫 `abandonDialogController`）的主按钮 action 从
`discardDraft()` 改成 `this.onClose()`——`requestClose` 已经先 `saveDraft` 了，这与文案
「已保存为草稿，下次编辑可恢复」一致。`discardDraft()` 因此没有调用方，两个 Dialog 里的
这个方法都已删除。真正"删草稿"的入口只剩设置里的「清除缓存」（文案已明示，见 §8.6，本次未改）。

**§9.13 之后**：编辑模式不再写草稿，这条文案在那里就是假话，因此拆出独立的
`discardEditDialogController`（「放弃修改？本次修改尚未保存，关闭后将丢失」）。
...
- **B3**：`beginSession` 不再排除 EDIT——编辑模式的图片按钮照常可用、
  ...
  **§9.13 反转**：编辑模式不再接草稿后，`beginSession` 对 EDIT **重新排除**（不登记活跃键、
  不装回媒体清单），两个 `...WithDraft` 上传入口对 EDIT 也**不落本地副本**。
...
- **确认框文案「放弃编辑？」与主按钮「放弃」**（新建模式）：行为恢复为"关掉编辑器、草稿保留"，
  文案保持原样；**编辑模式的文案已按 §9.13 换成「放弃修改？本次修改尚未保存，关闭后将丢失」**。
...
**（当时的）修法**：编辑分支改成与新回复分支同一判据——**该判据已被 §9.13 反转，见下一节**。
...
### 9.13 判据反转（用户决定）：编辑模式**不接草稿**，服务端是权威

**决定**（用户原话的语义）：发帖 / 新回复**没有服务端数据**，草稿是唯一来源；修改帖子 /
修改回复**有服务端数据**，以服务端为准，**不使用草稿数据**，同时注意草稿数据的管理。

**为什么 §9.12 的做法要撤回**：草稿的基线可能早已过期（网页版、另一台设备、版主操作都可能
改过这一层），拿本地残骸整串覆盖服务端内容，等于在用户不知情的情况下把别人对这篇帖子的修改
顶掉。`subject`（标题）一直是按这条原则处理的（`ensureEditSubject`：只认 `post/check` 回传值，
刻意不做本地兜底），正文没有理由用另一套判据。

**改法：读、写、媒体三条一起断**（只断一条就会留下"永远不会被读的假草稿"）。

| 位置 | 改动 |
|---|---|
| `ReplyDialog.aboutToAppear`（EDIT 分支） | 正文 = `initialContent`（服务端原文），**不读草稿**；匿名初始态另存 `initialAnonymity` 供"改没改"判定 |
| `ReplyDialog.requestClose`（EDIT 分支） | **不写草稿**；`hasUnsavedEdit()`（正文 / 匿名 / 上传在途）为假时**直接关闭、不弹框**，为真时弹 `discardEditDialogController`：「放弃修改？本次修改尚未保存，关闭后将丢失」 |
| `ReplyManager.saveDraft` / `loadDraft` | 对 `ReplyType.EDIT` 直接拦截（防御性：写下去也没人读，却会留下假草稿并把设置里的「草稿占用」带偏） |
| `ReplyManager.beginSession` | EDIT **不登记活跃键、不装回 `sessionMedia`** |
| `ReplyManager.uploadImageWithDraft` / `uploadMovingPhotoWithDraft` | EDIT 上传照做（本次会话内提交仍有效），但**不落本地副本** |
| `ReplyManager.discardEditDraft`（新增）+ `startEdit` | 进入编辑时**回收遗留**的 `draft_<tid>_edit_<pid>_<uid>` 草稿与 `draft_media/<key>/` 副本——历史版本写过它们，不回收就要等 7 天保留期，期间既占磁盘又计入「草稿占用」 |
| 新建侧（回复 / 贴条 / 发新主题） | **不动**：没有服务端数据，草稿仍是唯一来源 |

**代价（已说明并由用户确认接受）**：编辑面板里未提交的修改**不跨会话**——关掉面板或进程被杀
就丢，包括上一次编辑里上传成功但没提交的图片（凭证随 `beginSession` 作废、正文又只认服务端，
只能重新上传）。**但它不再是静默损坏**：编辑器里显示的就是将要提交的内容，用户看得见图不在；
"编辑器显示一份、提交或落盘另一份"这种混合态在编辑模式下已经不可能出现。

**验收**：§6 清单第 24–26 条（24 反向确认不读草稿与文案正确、25 同会话内编辑插图仍可发、
26 无修改不弹框 / 只改匿名要弹框）。
